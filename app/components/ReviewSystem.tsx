...
const CATERING_WA   = 'https://wa.me/2348094946923?text=Hi%20Chizzychops%20%26%20Grillz!%20I%27d%20like%20to%20enquire%20about%20*Event%20Catering*%20%F0%9F%8D%BD%EF%B8%8F%0A%0APlease%20share%20your%20catering%20packages%20and%20pricing.'
      ...
      <div className="container">
        ...
        {tab==='review' && (
          <>
            <div style={{textAlign:'center',marginBottom:'3rem'}}>
              ...
              <p style={{color:'rgba(255,255,255,0.45)',fontSize:'0.9375rem',maxWidth:'28rem',margin:'0 auto'}}>Your feedback helps us improve and helps others discover great food.</p>
            </div>
            ...
            <div style={{
              ...
              <div style={{display:'flex',gap:'0.75rem',alignItems:'center',flex:1}}>
                <GoogleIcon />
                <div>
                  <p style={{color:'#fff',fontWeight:700,fontSize:'0.9375rem'}}>Leave a Google Review</p>
                  <p style={{color:'rgba(255,255,255,0.4)',fontSize:'0.8125rem'}}>Visible publicly on Google Maps & Search</p>
                </div>
              </div>
              <div style={{display:'flex',gap:'0.75rem',width: isMobile ? '100%' : 'auto', flexWrap:'wrap'}}>
                ...
                <a href={GOOGLE_MAP} target="_blank" rel="noopener noreferrer"
                  style={{
                    ...
                    fontWeight:700,fontSize:'0.8125rem',textDecoration:'none',transition:'all 0.2s',
                    flexShrink:0, flex: isMobile ? 1 : 'unset', justifyContent:'center',
                  }}>
                  View on Maps
                </a>
              </div>
            ...
            {done ? (
              <div style={{textAlign:'center',padding:'3rem',borderRadius:'1.25rem',border:'1px solid rgba(37,211,102,0.2)',background:'rgba(37,211,102,0.05)'}}>
                ...
                <p style={{color:'rgba(255,255,255,0.5)',marginBottom:'1.5rem'}}>Your review has been sent to us. We read and appreciate every one!</p>
              ...
              <div style={{borderRadius:'1.25rem',border:'1px solid rgba(255,255,255,0.08)',background:'rgba(255,255,255,0.02)',overflow:'hidden'}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',borderBottom:'1px solid rgba(255,255,255,0.07)'}}>
                  {['Basic Info','Ratings','Your Review'].map((l,i)=>(
                    <div key={l} style={{padding: isMobile ? '0.75rem 0.5rem' : '1rem',textAlign:'center',fontSize: isMobile ? '0.6875rem' : '0.8125rem',fontWeight:700,
                ...
                <div style={{padding: isMobile ? '1.25rem' : '2rem'}}>
                  {step===1 && (
                    <div style={{display:'flex',flexDirection:'column',gap:'1.125rem'}}>
                      <div>
                        <label style={{color:'rgba(255,255,255,0.5)',fontSize:'0.8125rem',fontWeight:700,display:'block',marginBottom:'0.5rem'}}>Your Name *</label>
                        <input className="form-input" value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Amaka O." />
                      </div>
                      <div>
                        <label style={{color:'rgba(255,255,255,0.5)',fontSize:'0.8125rem',fontWeight:700,display:'block',marginBottom:'0.5rem'}}>What did you order? *</label>
                      ...
                      <div>
                        <label style={{color:'rgba(255,255,255,0.5)',fontSize:'0.8125rem',fontWeight:700,display:'block',marginBottom:'0.75rem'}}>Overall Rating *</label>
                      ...
                      <button className="btn-primary" disabled={!name||!dish||!overall} onClick={()=>setStep(2)} style={{opacity:name&&dish&&overall?1:0.35,cursor:name&&dish&&overall?'pointer':'not-allowed'}}>
                        Continue →
                      </button>
                    </div>
                  )}
                  ...
                  {step===2 && (
                    <div style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
                      ...
                      <div>
                        <label style={{color:'rgba(255,255,255,0.5)',fontSize:'0.8125rem',fontWeight:700,display:'block',marginBottom:'0.75rem'}}>Would you recommend us?</label>
                        <div style={{display:'flex',gap:'0.75rem'}}>
                          {[true,false].map(v=>(
                            <button key={String(v)} onClick={()=>setRecommend(v)} style={{flex:1,padding:'0.75rem',borderRadius:'0.75rem',border:'1.5px solid',fontWeight:700,fontSize: isMobile ? '0.8125rem' : '0.9rem',cursor:'pointer',transition:'all 0.2s',
                  ...
                  {step===3 && (
                    <div style={{display:'flex',flexDirection:'column',gap:'1.125rem'}}>
                      <div>
                        <label style={{color:'rgba(255,255,255,0.5)',fontSize:'0.8125rem',fontWeight:700,display:'block',marginBottom:'0.5rem'}}>Your Review * <span style={{color:'rgba(255,255,255,0.25)',fontWeight:400}}>(min. 20 characters)</span></label>
                      ...
                      <div style={{display:'flex',gap:'0.75rem'}}>
                        ...
                        <button className="btn-primary" onClick={submitReview} disabled={text.length<20} style={{flex:2,opacity:text.length>=20?1:0.35,cursor:text.length>=20?'pointer':'not-allowed'}}>
        ...
        {tab==='catering' && (
          <>
            ...
            {/* Catering packages — responsive grid */}
            <div style={{
              display:'grid',
              ...
              {cateringPackages.map(p=>{
                const isSelected = selectedPackage === p.name
                return (
                <div key={p.name} onClick={()=>setSelectedPackage(isSelected ? null : p.name)} style={{
                  padding:'1.5rem',
                  borderRadius:'1rem',
                  border:'1.5px solid',
                  borderColor: isSelected ? '#F97316' : p.highlight ? 'rgba(249,115,22,0.4)' : 'rgba(255,255,255,0.08)',
                  background: isSelected ? 'rgba(249,115,22,0.13)' : p.highlight ? 'rgba(249,115,22,0.07)' : 'rgba(255,255,255,0.03)',
                  position:'relative',
                  cursor:'pointer',
                  ...
                  {p.highlight && (
                    <div style={{
                      ...
                      background: isSelected ? 'linear-gradient(135deg,#22c55e,#15803d)' : 'linear-gradient(135deg,#F97316,#DC2626)',
                      color:'#fff',fontSize:'0.625rem',fontWeight:800,padding:'0.25rem 0.75rem',
                      borderRadius:'9999px',letterSpacing:'0.08em',whiteSpace:'nowrap',
                    }}>{isSelected ? 'SELECTED ✓' : 'MOST POPULAR'}</div>
                  )}
                  {isMobile ? (
                    <div style={{display:'flex',gap:'1rem',alignItems:'flex-start'}}>
                      <div style={{fontSize:'2rem',flexShrink:0,marginTop:'2px'}}>{p.icon}</div>
                      <div style={{flex:1}}>
                        <p style={{color:'#fff',fontFamily:'var(--font-playfair)',fontWeight:700,fontSize:'1rem',marginBottom:'0.125rem'}}>{p.name}</p>
                        <p style={{color:'rgba(249,115,22,0.8)',fontSize:'0.8125rem',fontWeight:600,marginBottom:'0.625rem'}}>{p.guests}</p>
            ...
            {cDone ? (
              <div style={{textAlign:'center',padding:'3rem',borderRadius:'1.25rem',border:'1px solid rgba(37,211,102,0.2)',background:'rgba(37,211,102,0.05)'}}>
                ...
                <p style={{color:'rgba(255,255,255,0.5)',marginBottom:'1.5rem'}}>We&apos;ve opened WhatsApp with your catering details. We&apos;ll get back to you with a full quote within 24 hours.</p>
              ...
              <div style={{
                ...
                {selectedPackage && (
                  <div style={{gridColumn:'1 / -1',display:'flex',alignItems:'center',gap:'0.625rem',padding:'0.75rem 1rem',borderRadius:'0.75rem',background:'rgba(249,115,22,0.08)',border:'1px solid rgba(249,115,22,0.25)'}}>
                    <span style={{fontSize:'0.8125rem',color:'rgba(249,115,22,0.9)',fontWeight:700}}>📦 Package selected:</span>
                    <span style={{fontSize:'0.875rem',color:'#fff',fontWeight:700}}>{selectedPackage}</span>
                    <button onClick={()=>setSelectedPackage(null)} style={{marginLeft:'auto',background:'none',border:'none',color:'rgba(255,255,255,0.35)',cursor:'pointer',fontSize:'0.8125rem',fontWeight:700}}>✕ Change</button>
                  </div>
                )}
                {[
                  ...
                  <div key={f.label}>
                    <label style={{color:'rgba(255,255,255,0.5)',fontSize:'0.8125rem',fontWeight:700,display:'block',marginBottom:'0.5rem'}}>{f.label}</label>
                    <input className="form-input" type={f.type} value={f.val} onChange={e=>f.set(e.target.value)} placeholder={f.placeholder} />
                  </div>
                ))}

                <div style={{gridColumn:'1 / -1'}}>
                  <label style={{color:'rgba(255,255,255,0.5)',fontSize:'0.8125rem',fontWeight:700,display:'block',marginBottom:'0.5rem'}}>Additional Notes / Menu Preferences</label>
                ...
                <div style={{
                  ...
                  <a href={CATERING_WA} target="_blank" rel="noopener noreferrer" className="btn-primary"
                    style={{
                      flex: isMobile ? 'unset' : 1,
                      justifyContent:'center',
                      textAlign:'center',
                      order: isMobile ? 2 : 1,  /* Secondary action goes below on mobile */
                    }}>
                    <WAIcon /> Quick WhatsApp Enquiry
                  </a>
                  <button className="btn-primary" onClick={submitCatering}
                    disabled={!cName||!cPhone||!cDate||!cGuests}
                    style={{
                      flex: isMobile ? 'unset' : 2,
                      opacity:cName&&cPhone&&cDate&&cGuests?1:0.35,
                      cursor:cName&&cPhone&&cDate&&cGuests?'pointer':'not-allowed',
                      background:'linear-gradient(135deg,#DC2626,#7C2D12)',
                      order: isMobile ? 1 : 2,  /* Primary action first on mobile */
                    }}>
                    <WAIcon /> Submit Full Catering Request
                  </button>
                </div>
              </div>
            )}
