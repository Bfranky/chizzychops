...
const services = [
  {
    icon: '🚚',
    title: 'Home Delivery',
    desc: 'We deliver your freshly cooked meal straight to your door across Lagos. Every order is made from scratch, so please allow up to 6 hours.',
    badge: 'Most Popular',
    badgeColor: '#F97316',
    highlight: true,
  },
  ...
  return (
    <section id="services" className="section" style={{ background: 'var(--brand-surface)' }}>
      <div className="container">
        ...
        {/* Online business badge */}
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '3.5rem' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.625rem', padding: '0.625rem 1.25rem', borderRadius: '9999px', background: 'rgba(249,115,22,0.1)', border: '1px solid rgba(249,115,22,0.25)' }}>
            <span style={{ width: '7px', height: '7px', borderRadius: '50%', background: '#4ADE80', display: 'inline-block', flexShrink: 0 }} />
            <span style={{ color: 'rgba(255,255,255,0.75)', fontSize: '0.8125rem', fontWeight: 700 }}>100% Online · Order via WhatsApp · Delivered Across Lagos</span>
          </div>
        </div>

        {/* Services — new design: alternating featured + grid */}
        {/* Featured delivery card */}
        <div style={{
          borderRadius: '1.25rem',
          border: '1.5px solid rgba(249,115,22,0.4)',
          background: 'linear-gradient(135deg, rgba(249,115,22,0.1), rgba(220,38,38,0.05))',
          padding: '2rem 2.5rem',
          ...
          <div style={{ flex: 1, minWidth: '14rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
              <h3 style={{ color: '#fff', fontFamily: 'var(--font-playfair)', fontWeight: 700, fontSize: '1.375rem', margin: 0 }}>Home Delivery</h3>
              <span style={{ background: '#F97316', color: '#fff', fontSize: '0.625rem', fontWeight: 800, padding: '0.25rem 0.75rem', borderRadius: '9999px', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Most Popular</span>
            </div>
            <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: '0.9375rem', lineHeight: 1.75, marginBottom: '1rem' }}>
              Your freshly home-cooked meal, delivered straight to your door anywhere across Lagos.
              Every single order is cooked from scratch — <strong style={{ color: '#FBBF24' }}>please allow up to 6 hours</strong> from order confirmation.
              Worth every minute.
            </p>
            <div style={{ display: 'flex', gap: '0.625rem', flexWrap: 'wrap' }}>
              <span style={{ background: 'rgba(251,191,36,0.12)', color: '#FBBF24', fontSize: '0.75rem', fontWeight: 700, padding: '0.375rem 0.875rem', borderRadius: '9999px', border: '1px solid rgba(251,191,36,0.2)' }}>⏱️ Up to 6hrs</span>
              <span style={{ background: 'rgba(74,222,128,0.1)', color: '#4ADE80', fontSize: '0.75rem', fontWeight: 700, padding: '0.375rem 0.875rem', borderRadius: '9999px', border: '1px solid rgba(74,222,128,0.2)' }}>📍 All Lagos</span>
              <span style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.6)', fontSize: '0.75rem', fontWeight: 700, padding: '0.375rem 0.875rem', borderRadius: '9999px', border: '1px solid rgba(255,255,255,0.1)' }}>✅ Always Fresh</span>
            </div>
          </div>
          <a href={WA} target="_blank" rel="noopener noreferrer"
            style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.875rem 1.75rem', borderRadius: '9999px', background: 'linear-gradient(135deg,#F97316,#DC2626)', color: '#fff', fontWeight: 800, fontSize: '0.9375rem', textDecoration: 'none', whiteSpace: 'nowrap', boxShadow: '0 4px 20px rgba(249,115,22,0.35)', flexShrink: 0, transition: 'transform 0.2s' }}
        ...
        {/* Other 4 services in a 2×2 grid */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 260px), 1fr))', gap: '1rem', marginBottom: '4rem' }}>
          {services.slice(1).map(s => (
            <div key={s.title} style={{
              ...
              <div style={{ width: '2.75rem', height: '2.75rem', borderRadius: '0.75rem', background: 'rgba(249,115,22,0.1)', border: '1px solid rgba(249,115,22,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.25rem', flexShrink: 0 }}>
                {s.icon}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.375rem', flexWrap: 'wrap' }}>
                  ...
                  {s.badge && (
                    <span style={{ background: s.badgeColor, color: '#fff', fontSize: '0.5625rem', fontWeight: 800, padding: '0.2rem 0.5rem', borderRadius: '9999px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{s.badge}</span>
                  )}
                </div>
        ...
        <div style={{ borderRadius: '1.25rem', border: '1px solid rgba(249,115,22,0.15)', overflow: 'hidden', background: 'rgba(249,115,22,0.04)' }}>
          <div className="why-grid">
            ...
            {/* Right */}
            <div style={{ padding: '2.5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', alignContent: 'start' }}>
              {reasons.map(r => (
                <div key={r.text} style={{ display: 'flex', gap: '0.625rem', alignItems: 'flex-start', padding: '0.875rem', borderRadius: '0.75rem', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <style>{`
        ...
        @media (max-width: 767px) {
          ...
          .why-grid > div:first-child { border-right: none !important; border-bottom: 1px solid rgba(255,255,255,0.07); }
