...
function Dashboard() {
  ...
  const loadOrders = async () => {
    setLoadO(true)
    try { const o = await fetchOrders(); setOrders(o); setNewCount(o.filter(x => x.status === 'new').length) }
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'orders' }, payload => {
        ...
        setNewCount(n => n + 1)
        showToast(`🆕 New order received! ${o.id}`)
        notifyOwnerWA(o)
        try { new Audio('/notification.mp3').play() } catch {}
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders' }, payload => {
        setOrders(prev => prev.map(x => x.id === payload.new.id ? payload.new as Order : x))
      })
      .subscribe()
  ...
  const notifyOwnerWA = (o: Order) => {
    const lines = (o.items as any[]).map(i => `• ${i.name} ×${i.qty} — ${fmt(i.price * i.qty)}`)
    const msg = [
      `🆕 *NEW ORDER — ${o.id}*`, '',
      ...
      '', `Manage: https://chizzychops.vercel.app/admin`,
    ].filter(Boolean).join('\n')
    window.open(`https://wa.me/${OWNER_WA}?text=${encodeURIComponent(msg)}`, '_blank')
  }

  const handleStatusChange = async (id: string, status: OrderStatus) => {
    try { await updateOrderStatus(id, status); showToast(`✅ Order ${id} → ${status}`) }
    catch (e: any) { showToast('❌ ' + e.message) }
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0300', color: '#fff', fontFamily: 'system-ui,sans-serif' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginTop: '2px', alignItems: 'center' }}>
              ...
              <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.75rem' }}>{orders.length} total orders</span>
            </div>
          </div>
        </div>
      ...
      {/* TABS */}
      <div style={{ background: '#0f0400', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '0 1.25rem', display: 'flex', gap: '0.25rem' }}>
        {TABS.map(t => (
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(130px,1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
        {[
          { label: "Today", value: todayCount, icon: '📅', color: '#60A5FA' },
          { label: 'Active', value: activeCount, icon: '🔥', color: '#F97316' },
          { label: 'New', value: orders.filter(o=>o.status==='new').length, icon: '🆕', color: '#DC2626' },
          { label: 'Delivered', value: orders.filter(o=>o.status==='delivered').length, icon: '✅', color: '#4ADE80' },
          { label: 'Revenue', value: fmt(revenue), icon: '💰', color: '#FBBF24' },
        ].map(s => (
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1rem', alignItems: 'center' }}>
        {(['all','new','confirmed','preparing','ready','delivered','cancelled'] as const).map(s => (
          <button key={s} onClick={() => setFilter(s)}
            style={{ padding: '0.375rem 0.875rem', borderRadius: '9999px', fontSize: '0.8125rem', fontWeight: 700, border: '1px solid', cursor: 'pointer', whiteSpace: 'nowrap',
              background: filter === s ? (s==='all' ? 'linear-gradient(135deg,#F97316,#DC2626)' : STATUS_CONFIG[s as OrderStatus]?.bg) : 'rgba(255,255,255,0.04)',
              color: filter === s ? (s==='all' ? '#fff' : STATUS_CONFIG[s as OrderStatus]?.color) : 'rgba(255,255,255,0.5)',
              borderColor: 'transparent',
            }}>{s === 'all' ? 'All Orders' : STATUS_CONFIG[s as OrderStatus].label}</button>
        ))}
      ...
      {loading ? <Loader text="Loading orders…" /> : displayed.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
          {displayed.map(order => {
            ...
            return (
              <div key={order.id} style={{ background: '#120600', border: `1px solid ${isNew ? 'rgba(96,165,250,0.35)' : 'rgba(255,255,255,0.07)'}`, borderRadius: '0.875rem', overflow: 'hidden', transition: 'border-color 0.2s' }}>
                {/* Header row */}
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', padding: '0.875rem 1rem', cursor: 'pointer', flexWrap: 'wrap' }} onClick={() => setExpanded(isExp ? null : order.id)}>
                  <span style={{ background: cfg.bg, color: cfg.color, fontSize: '0.6875rem', fontWeight: 800, padding: '0.2rem 0.625rem', borderRadius: '9999px', border: `1px solid ${cfg.color}25`, whiteSpace: 'nowrap' }}>{cfg.label}</span>
                ...
                {isExp && (
                  <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '1rem' }}>
                    ...
                    <div style={{ marginBottom: '0.875rem' }}>
                      <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.6875rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.5rem' }}>Order Items</p>
                      {(order.items as any[]).map((item, i) => (
                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.3rem 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                          <span style={{ color: '#fff', fontSize: '0.875rem' }}>{item.name} <span style={{ color: 'rgba(255,255,255,0.4)' }}>×{item.qty}</span></span>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
                      {cfg.next && (
                        <button onClick={() => onStatusChange(order.id, cfg.next!)} style={{ ...btnPrimary, fontSize: '0.8125rem', padding: '0.5rem 1rem' }}>
                          → {STATUS_CONFIG[cfg.next].label}
                        </button>
                      )}
                      {!['delivered','cancelled'].includes(order.status) && (
                        <button onClick={() => onStatusChange(order.id, 'cancelled')} style={{ background: 'rgba(220,38,38,0.1)', border: '1px solid rgba(220,38,38,0.2)', color: '#F87171', padding: '0.5rem 1rem', borderRadius: '9999px', cursor: 'pointer', fontSize: '0.8125rem', fontWeight: 700 }}>Cancel</button>
                      )}
                      {order.customer_phone && (
                        <a href={`https://wa.me/${order.customer_phone.replace(/\D/g,'')}?text=${encodeURIComponent(`Hi! Your Chizzychops order ${order.id} is now ${order.status}. 🍽️`)}`}
                          ...
                          style={{ background: 'rgba(37,211,102,0.1)', border: '1px solid rgba(37,211,102,0.2)', color: '#4ADE80', padding: '0.5rem 1rem', borderRadius: '9999px', fontSize: '0.8125rem', fontWeight: 700, textDecoration: 'none' }}>
                          📲 WhatsApp Customer
                        </a>
                      )}
...
// MENU PANEL
// ══════════════════════════════════════════════════════════
function MenuPanel({ items, loading, onRefresh, showToast }: {
  items: MenuItem[]; loading: boolean; onRefresh: () => void; showToast: (m: string) => void
}) {
  ...
  return (
    <div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(130px,1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
        {CATEGORIES.map(c => (
          <div key={c} style={{ background: '#120600', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '0.75rem', padding: '0.875rem' }}>
            <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.6875rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{c}</p>
            <p style={{ color: '#F97316', fontWeight: 900, fontSize: '1.5rem', lineHeight: 1.2, marginTop: '0.25rem' }}>{items.filter(i=>i.category===c).length}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1rem', alignItems: 'center' }}>
        ...
        {['All',...CATEGORIES].map(c => (
          <button key={c} onClick={() => setCatF(c)}
            style={{ padding: '0.375rem 0.875rem', borderRadius: '9999px', fontSize: '0.8125rem', fontWeight: 700, border: '1px solid', cursor: 'pointer',
              background: catF === c ? 'linear-gradient(135deg,#F97316,#DC2626)' : 'rgba(255,255,255,0.04)',
              color: catF === c ? '#fff' : 'rgba(255,255,255,0.5)',
              borderColor: 'transparent',
            }}>{c}</button>
        ))}
        <button onClick={() => setEditing({ ...EMPTY_ITEM, id: slug(Date.now().toString()), sort_order: items.length + 1 })}
          style={{ ...btnPrimary, fontSize: '0.8125rem', padding: '0.5rem 1.125rem', marginLeft: 'auto' }}>+ Add Item</button>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '600px' }}>
            <thead>
              <tr style={{ background: 'rgba(255,255,255,0.03)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                {['Img','Name','Category','Price','Badge',''].map((h,i) => (
                  <th key={i} style={{ padding: '0.75rem 1rem', textAlign: 'left', color: 'rgba(255,255,255,0.35)', fontSize: '0.6875rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map((item, idx) => (
                <tr key={item.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}
                  ...
                  <td style={{ padding: '0.625rem 1rem' }}>
                    ...
                    <img src={item.img_url} alt="" style={{ width: '44px', height: '44px', objectFit: 'cover', borderRadius: '0.5rem', border: '1px solid rgba(255,255,255,0.08)' }} onError={e=>{(e.target as HTMLImageElement).src='https://placehold.co/44x44/1A0800/F97316?text=?'}} />
                  </td>
                  <td style={{ padding: '0.625rem 1rem' }}>
                    <p style={{ color: '#fff', fontWeight: 700, fontSize: '0.875rem' }}>{item.name}</p>
                  ...
                  <td style={{ padding: '0.625rem 1rem', color: 'rgba(255,255,255,0.5)', fontSize: '0.8125rem', whiteSpace: 'nowrap' }}>
                    {item.category}{item.subcat && <span style={{ color: '#F97316', display: 'block', fontSize: '0.75rem' }}>{item.subcat}</span>}
                  </td>
                  <td style={{ padding: '0.625rem 1rem', color: '#FBBF24', fontWeight: 800, whiteSpace: 'nowrap' }}>{fmt(item.price)}</td>
                  <td style={{ padding: '0.625rem 1rem' }}>
                    {item.badge && <span style={{ background: item.badge_color||'#F97316', color: '#fff', fontSize: '0.6rem', fontWeight: 800, padding: '0.2rem 0.5rem', borderRadius: '9999px', textTransform: 'uppercase' }}>{item.badge}</span>}
                  </td>
                  <td style={{ padding: '0.625rem 1rem' }}>
...
function StatsPanel({ orders, menuItems }: { orders: Order[]; menuItems: MenuItem[] }) {
  const topItems = orders.flatMap(o => o.items as any[]).reduce((acc: Record<string,any>, item) => {
    if (!acc[item.id]) acc[item.id] = { name: item.name, qty: 0, revenue: 0 }
    acc[item.id].qty     += item.qty
    acc[item.id].revenue += item.price * item.qty
    return acc
  }, {})
  const topList = Object.values(topItems).sort((a,b) => b.qty - a.qty).slice(0, 8)
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(140px,1fr))', gap: '0.75rem' }}>
          {(Object.entries(STATUS_CONFIG) as [OrderStatus, typeof STATUS_CONFIG[OrderStatus]][]).map(([k,v]) => (
            <div key={k} style={{ background: v.bg, border: `1px solid ${v.color}25`, borderRadius: '0.875rem', padding: '1rem' }}>
              <p style={{ color: v.color, fontWeight: 900, fontSize: '2rem', lineHeight: 1 }}>{orders.filter(o=>o.status===k).length}</p>
          ...
          <div style={{ background: '#120600', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '1rem', overflow: 'hidden' }}>
            {topList.map((item, i) => (
              <div key={item.name} style={{ display: 'flex', gap: '1rem', alignItems: 'center', padding: '0.875rem 1.25rem', borderBottom: i < topList.length-1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                <span style={{ color: '#F97316', fontWeight: 900, fontSize: '1.125rem', minWidth: '24px' }}>#{i+1}</span>
...
function EditModal({ item, onClose, onSaved }: { item: Omit<MenuItem,'updated_at'>; onClose: () => void; onSaved: () => void }) {
  ...
  const handleUpload = async (file: File, field: 'img_url' | 'img2_url') => {
    const setU = field === 'img_url' ? setUpImg : setUpImg2
    setU(true)
  ...
  const save = async () => {
    if (!form.id.trim())       return setError('ID required')
    if (!form.name.trim())     return setError('Name required')
    if (form.price <= 0)       return setError('Price must be > 0')
    if (!form.img_url?.trim()) return setError('Main image required')
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 'min(680px,calc(100vw - 2rem))', maxHeight: '90vh', overflowY: 'auto', background: '#1A0800', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1.25rem', zIndex: 201, padding: '2rem', boxShadow: '0 24px 80px rgba(0,0,0,0.7)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          ...
          <Field label="Category *">
            <select value={form.category} onChange={e=>set('category',e.target.value)} style={inputStyle}>{CATEGORIES.map(c=><option key={c}>{c}</option>)}</select>
          </Field>
          <Field label="Sub-category">
            <select value={form.subcat||''} onChange={e=>set('subcat',e.target.value)} style={inputStyle}>{SUBCATS.map(s=><option key={s} value={s}>{s||'— None —'}</option>)}</select>
          </Field>
          ...
          <Field label="Badge Colour">
            <div style={{ display:'flex', gap:'0.5rem', alignItems:'center' }}>
              <input value={form.badge_color||''} onChange={e=>set('badge_color',e.target.value)} style={{ ...inputStyle, flex:1 }} placeholder="#F97316" />
          ...
          <Field label="Main Image *" span>
            <div style={{ display:'flex', gap:'0.75rem', alignItems:'flex-start' }}>
              <div style={{ flex:1 }}>
                <input value={form.img_url||''} onChange={e=>set('img_url',e.target.value)} style={{ ...inputStyle, marginBottom:'0.5rem' }} placeholder="/soup/egusi.jpg or https://…" />
                <div style={{ display:'flex', gap:'0.5rem' }}>
                  <input ref={img1} type="file" accept="image/*" style={{ display:'none' }} onChange={e=>e.target.files?.[0]&&handleUpload(e.target.files[0],'img_url')} />
                  <button onClick={()=>img1.current?.click()} disabled={upImg} style={{ ...btnSecondary, fontSize:'0.8rem', padding:'0.375rem 0.75rem', opacity:upImg?0.7:1 }}>{upImg?'⏳ Uploading…':'📁 Upload'}</button>
                </div>
              </div>
          ...
          <Field label="Second Image" hint="optional — crossfades" span>
            <div style={{ display:'flex', gap:'0.75rem', alignItems:'flex-start' }}>
              <div style={{ flex:1 }}>
                <input value={form.img2_url||''} onChange={e=>set('img2_url',e.target.value)} style={{ ...inputStyle, marginBottom:'0.5rem' }} placeholder="/soup/oha-2.jpg (optional)" />
                <div style={{ display:'flex', gap:'0.5rem' }}>
                  <input ref={img2} type="file" accept="image/*" style={{ display:'none' }} onChange={e=>e.target.files?.[0]&&handleUpload(e.target.files[0],'img2_url')} />
                  <button onClick={()=>img2.current?.click()} disabled={upImg2} style={{ ...btnSecondary, fontSize:'0.8rem', padding:'0.375rem 0.75rem', opacity:upImg2?0.7:1 }}>{upImg2?'⏳ Uploading…':'📁 Upload'}</button>
...
const inputStyle: React.CSSProperties = {
  ...
  color:'#fff', borderRadius:'0.625rem', padding:'0.6875rem 0.875rem', fontSize:'0.9rem',
  outline:'none', fontFamily:'system-ui,sans-serif', boxSizing:'border-box',
}
const btnPrimary: React.CSSProperties = {
  background:'linear-gradient(135deg,#F97316,#DC2626)', color:'#fff', fontWeight:800,
  fontSize:'0.9375rem', padding:'0.8125rem 1.75rem', borderRadius:'9999px', border:'none',
  cursor:'pointer', display:'inline-flex', alignItems:'center', justifyContent:'center', gap:'0.4rem',
}
const btnSecondary: React.CSSProperties = {
  background:'rgba(255,255,255,0.06)', color:'#fff', fontWeight:700, fontSize:'0.9375rem',
  padding:'0.8125rem 1.75rem', borderRadius:'9999px', border:'1px solid rgba(255,255,255,0.12)',
